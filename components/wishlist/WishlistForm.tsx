...
export function WishlistForm() {
  ...
  // Shown right after a successful add — a Looking For request is often
  // exactly the flip side of a trade (you're missing a card someone else
  // might have), so this offers the natural next step instead of leaving
  // the user to go find /collection/add on their own.
  ...
  return (
    <>
      {justAdded && (
        <div className="flex flex-wrap items-center justify-between gap-3 rounded-lg border border-primary/40 bg-primary/10 p-4">
          <p className="text-sm text-text">
            Added to Looking For! Got a card to offer in return?
          </p>
          <Link href="/collection/add?status=for_trade" className="btn-primary shrink-0">
            <Plus className="h-4 w-4" />
            Offer a card to trade
          </Link>
        </div>
      )}
